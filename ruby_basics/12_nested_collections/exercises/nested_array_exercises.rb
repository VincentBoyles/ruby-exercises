...
def count_empty_seats(chart)
  ...
  empty = 0
  chart.each_with_index do |seats, index| 
    seats.each_with_index do |seats|
     if seats.nil? 
       empty += 1
     end
    end
...
def find_favorite(array_of_hash_objects)
  ...
  check_fav = check_arr.any? { |hash| hash[:is_my_favorite?] }
  if check_fav
    array_of_hash_objects.each do |hash|
      return hash if hash.dig(:is_my_favorite?)
    end
  else
    return nil
  end
